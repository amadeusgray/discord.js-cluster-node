...
class Cluster {
	...
	constructor(client, { debug }) {
		...
		console.log = (str) => process.send({
			name: "log",
			msg: this.logOverride(str)
		});
		console.error = (str) => process.send({
			name: "error",
			msg: this.logOverride(str)
		});
		console.warn = (str) => process.send({
			name: "warn",
			msg: this.logOverride(str)
		});
		console.info = (str) => process.send({
			name: "info",
			msg: this.logOverride(str)
		});
		console.debug = (str) => process.send({
			name: "debug",
			msg: this.logOverride(str)
		});
	...
	connect(firstShardID, lastShardID, maxShards, token, type, clientOptions) {
		...
		bot.on("connect", id => {
			process.send({
				name: "log",
				msg: `La shard ${id} ha establecido conexión.`
			});
		});
		...
		bot.on("shardDisconnect", (err, id) => {
			process.send({
				name: "log",
				msg: `La shard ${id} se ha desconectado.`
			});
			let embed = {
	...
	loadCode(bot) {
		...
		if (app.prototype instanceof Base) {
			...
			console.error("Tu código no se ha cargado. Esto es porque no has extendido a la clase Base. Por favor extiende a la clase Base");
		}
	}
	...
	startStats(bot) {
		setInterval(() => {
			...
			this.users = bot.guilds.cache.reduce((prev, guild) => prev + guild.memberCount, 0)
			this.uptime = bot.uptime;
			this.channels = bot.channels.cache.size;
			this.largeGuilds = bot.guilds.cache.filter(g => g.large).size;
			this.exclusiveGuilds = bot.guilds.cache.filter(g => g.partner).size;
			this.shardsStats = [];
